...
- TanStack Form
- TanStack Router
- Zod contracts from `@rolf-sales-rep-mvp/contracts`
- shadcn CLI
- Playwright
- ESLint
...
The run starts Docker Compose `postgres_test`, applies migrations to `rolf_sales_rep_mvp_test`, starts the backend with `TEST_DATABASE_URL` as its `DATABASE_URL`, starts Vite, and removes the test database volume after the run by default.
