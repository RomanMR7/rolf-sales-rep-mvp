  ...
  const authService = {
    async authenticateAccessToken(accessToken: string | undefined) {
      ...
      return {
        ...
        permissions: {
          usersManage: role === 'OWNER' || role === 'ADMIN',
          functionsManage: role === 'OWNER' || role === 'ADMIN',
          scriptsManage: role === 'OWNER' || role === 'ADMIN',
          metricsView: true,
          leadsManage: role === 'OWNER' || role === 'ADMIN' || role === 'MANAGER',
          leadsView: role !== 'VIEWER',
        },
        navigation: {
          owner: role === 'OWNER',
          dashboard: true,
          leads: role !== 'VIEWER',
          managers: role === 'OWNER' || role === 'ADMIN' || role === 'SUPERVISOR',
          metrics: true,
          functions: role === 'OWNER' || role === 'ADMIN',
          scripts: role === 'OWNER' || role === 'ADMIN',
          settings: true,
        },
        effectiveSession: {
          mode: null,
